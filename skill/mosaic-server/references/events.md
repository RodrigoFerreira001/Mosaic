...
| `trigger` | `EventTrigger` | required | The condition that must occur for this event to run — see §6 "Event Chaining" below. |
| `events` | `List<EventSchema>?` | `null` | This event's children, each declaring which of *this* event's outgoing triggers runs it. |

For the underlying mechanics — trigger matching, `incomingData` propagation, the `field ?: incomingData` fallback convention, `GetScreen`/`ChangeScreenState`'s split — see §6 "Event Chaining" and §7 "Data Flow" in the main `SKILL.md`. This reference documents *what* each event does, *what parameters it takes*, and *what triggers it fires*, not the dispatch machinery itself.
...
**Notes:** the value sent is `data` when it's non-null, otherwise `incomingData` — the `field ?: incomingData` fallback convention (see §7 "Data Flow").
...
| Parameter | Type | Default | What it's for |
|---|---|---|---|
| `cancellableEventId` | `String` | required | Key the job is registered under — a **global namespace**, not scoped per screen — a global `CancellableEventsHolder` keyed by this id, see the `mosaic-client` skill's explicit-mechanisms table. |
...
| `navigatorId` | `String` | required (no default anywhere in the framework) | Which registered navigator's back stack to push onto (`"root"` is the app's own, by convention). |
...
**Notes:** `incomingData` is **required** — any non-null value is accepted as-is, no type validation. See the `mosaic-client` skill's explicit-mechanisms table for `NetworkParametersHolder`'s "whoever consumes it first wins" behavior — this event only stages the value, it doesn't guarantee which subsequent network event reads it.
...
**Notes:** dismissal here starts the two-phase animation handshake described in §6 "Event Chaining" — the overlay isn't actually gone from the tree the instant this event succeeds.
...
**Notes:** `incomingData` not consumed. Unlike the two bottom sheet variants, `Dialog`'s dismissal has **no exit animation to wait for** — its two-phase handshake resolves instantly.
...
Stops the loading indicator of the `PullToRefresh` tile identified by `tileId`. The tile never hides its own spinner — this event has to close every refresh flow, on the success branch and the failure branch alike (see `references/tiles.md`'s `PullToRefresh` entry).
...
**Notes:** the request always targets **this screen's own id** — there's no way to fetch a different screen's content through this event; fetching another screen is always navigation (`Navigate`/`NavigateClearingStack`). This is exactly the pair the `initialEvents` default of every `entry{}` uses. `incomingData` not consumed.
...
**Notes:** `incomingData` not consumed. `Rationale` is Android-only in practice (per platform permission APIs); other platforms only ever produce `Granted`/`Denied`.
...
`updateData` is one of: `incomingTileUpdateData()` (uses `incomingData` as-is when it's a map), `inlineTileUpdateData(map)` / `inlineTileUpdateData(vararg pairs)` (a literal map), or `mappedIncomingTileUpdateData(patterns)` (one template per field, each resolved against `incomingData` via the `<|path|>` template engine).
